...
class JetCombiner {
  ...
  public:
  JetCombiner(bool trainMVA, float Luminosity, const vector<Dataset*>& datasets, string MVAMethod = "Likelihood", bool Tprime = false, string MVAfilePostfix = "", string postfix = "", int mode=1);
    ~JetCombiner();
    ...
    void ProcessEvent_SingleHadTop(Dataset* dataSet, const vector<TRootMCParticle*> mcParticles, const vector<TRootJet*> selectedJets, const TRootGenEvent* genEvt, float scaleFactor=1);
